...
static bool findVariables(void *data, obs_source_t *source)
{
	...
	if (!strcmp("text_ft2_source_mustache_v2", id) ||
		...
		wstring text(mySource->text);
		blog(LOG_INFO,
		     "findVariables: found text_ft2_source_mustache_v2 source with text %s",
		     QString::fromStdWString(mySource->text)
			     .toStdString()
			     .c_str());
		const auto variables_begin = wsregex_iterator(
			text.begin(), text.end(), variable_regex);
					   ...
					   void *)
{
	...
	if (saving) {
		...
		obs_data_set_obj(save_data, "text-freetype2-mustache", obj);
		// variablesAndValues->clear();
		blog(LOG_INFO,
		     "SaveOBSTextMustacheDefinitions: Done saving data");
	} else {
		OBSDataAutoRelease obj =
			obs_data_get_obj(save_data, "text-freetype2-mustache");
...
{
	blog(LOG_INFO, "OBSEvent: %d", event);
	switch (event) {
...
extern "C" void InitOBSTextMustacheDefinitions()
{
	QAction *const action = (QAction *)obs_frontend_add_tools_menu_qaction(
		obs_module_text("TextFreetype2Definitions"));
